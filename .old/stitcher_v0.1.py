...
def create_cross_stitch_pdf(image_path, key_file_path):
...
def bundle_files_into_zip(image_path, pdf_path, key_file_path):
    ...
    if pixelation_option:
        ...
        for size in sizes:
            ...
            if cross_stitch_option:
                ...
                cross_stitch_img_path, key_file_path = create_cross_stitch_image(resized_image, dmc_color_map, symbol_size=20, max_colors=max_colors)
                pdf_filename = create_cross_stitch_pdf(cross_stitch_img_path, key_file_path)
                zip_path = bundle_files_into_zip(cross_stitch_img_path, pdf_filename, key_file_path)
        ...
        if cross_stitch_option:
            dmc_color_map = load_dmc_colors("DMC_RGB.csv")  # Load DMC colors from the provided CSV file
            cross_stitch_img_path, key_file_path = create_cross_stitch_image(image, dmc_color_map, symbol_size=20, max_colors=max_colors)
            pdf_filename = create_cross_stitch_pdf(cross_stitch_img_path, key_file_path)
            zip_path = bundle_files_into_zip(cross_stitch_img_path, pdf_filename, key_file_path)
            
            st.image(Image.open(cross_stitch_img_path), caption='Cross Stitch Pattern', use_column_width=True)
