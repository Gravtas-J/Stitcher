...
def create_key_pdf(key_file_path):
    ...
    # Update the path to where you have a valid font available on your system
...
def create_key_rtf(key_file_path):
    rtf_key_filename = os.path.join(tempfile.gettempdir(), "color_key.rtf")
    with open(key_file_path, 'r', encoding='utf-8') as key_file:
        lines = key_file.readlines()
    
    rtf_content = "{\\rtf1\\ansi\n"
    for line in lines:
        rtf_content += line.strip() + "\\line\n"
    rtf_content += "}"
    
    with open(rtf_key_filename, 'w', encoding='utf-8') as rtf_file:
        rtf_file.write(rtf_content)
    
    return rtf_key_filename
...
def bundle_files_into_zip(image_path, pdf_path, key_file_path, key_pdf_path, key_rtf_path):
    ...
    if pixelation_option:
        ...
        for size in sizes:
            ...
            if cross_stitch_option:
                ...
                zip_path = bundle_files_into_zip(combined_image_path, pdf_filename, key_file_path, key_pdf_filename, key_rtf_filename)
